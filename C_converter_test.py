...
from C_converter import Converter, convert
...
def test_converter(value, sys_from, sys_to, func):
    assert convert(value, sys_from, sys_to) == func(value)
...
def test_celsius_to_kelvin_fixed():
    assert round(Converter.celsius_to_kelvin(225), 2) == 498.15


def test_celsius_to_fahrenheit_fixed():
    assert round(Converter.celsius_to_fahrenheit(134), 2) == 273.2


def test_kelvin_to_fahrenheit_fixed():
    assert round(Converter.kelvin_to_fahrenheit(-6913), 2) == -12903.07


def test_kelvin_to_celsius_fixed():
    assert round(Converter.kelvin_to_celsius(3322), 2) == 3048.85
...
def test_fahrenheit_to_celsius_fixed():
    assert round(Converter.fahrenheit_to_celsius(2264), 2) == 1240
